...
import {
  getGameCoursesPaginated,
  batchDeleteGameCourses,
...
export default function GameCoursesPage() {
  const { user } = useAuth();
  const [courses, setCourses] = useState<GameCourse[]>([]);
  ...
  const loadCourses = async () => {
    setLoading(true);
    try {
      const result = await getGameCoursesPaginated(
        page,
        limit,
        searchQuery,
        filterStatus,
      ...
      if (result.success) {
        setCourses(result.data as GameCourse[]);
        setTotal(result.total);
        setTotalPages(result.totalPages);
      } else {
        toast.error("Failed to load game courses");
      }
    } catch (error) {
      toast.error("Failed to load game courses");
      console.error(error);
  ...
  const handleBatchDelete = async () => {
    ...
    setDeleting(true);
    try {
      const result = await batchDeleteGameCourses(selectedIds, user.uid);
      if (result.success) {
  ...
  return (
    <div>
      <PageHeader
        title="Game Courses"
        subtitle="Manage your game courses"
        actions={
          <Link
            href="/admin/game-courses/new"
      ...
      {courses.length === 0 ? (
        <EmptyState
          ...
          title="No game courses yet"
          description="Create your first game course to get started."
          action={
            <Link
              href="/admin/game-courses/new"
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <Link
                      href={`/admin/game-courses/${course.id}`}
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  />
                  <Badge
                    label={course.gameType === "uploaded" ? "Uploaded" : "URL"}
                    variant={course.gameType === "uploaded" ? "info" : "purple"}
                  />
                ...
                {courses.map((course) => (
                  <tr key={course.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <Badge
                        label={
                          course.gameType === "uploaded" ? "Uploaded" : "URL"
                        }
                        variant={
                          course.gameType === "uploaded" ? "info" : "purple"
                        }
                      />
                    </td>
